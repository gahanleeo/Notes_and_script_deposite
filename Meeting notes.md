...
Working on TCGA sample, Oscar shows me the series of step to make into final imputed vcf files.
Now, we just finished final step to phased imputed vcf files, currenly extract those SNPs of TERT region and check the quality of imputatiuon. and then do accosiaiton brwteen those existed mutation signature  data. 
...
HichIP, 
Novaseq-SP flowcell kit, have TEAM chat room whcih has a list of files 
...
how to visualized  region compare between methylation 
...
cf(QTL)
_Quantitative trait locus_ (QTL) analysis is a statistical method that links two types of information—phenotypic data (trait measurements) and genotypic data
...
- Working on TCGA sample, 

 just finished final all the imputed data in 33 cancer, and extract 12 SNPs of TERT region and check the quality of imputatiuon. We also tried another imputatiuon tool Michigan imputation server to check our imputation quality with theirs. 

the plot here shows the degree of argreement between ours and michigan, the value is around 0.6-0.9

Currently trying to run other cancer type in Michigan Server, Since Michigian server takes long time to run, also they will have error when some sample with low SNP calling 

Meanwhile we can also try to run association analysis for mutation signature data with our imputated data 




- TCGA Burket lymphpma WGS test download, download slice of bam files

- F/M code need consist 
- 

---
### 0601

- Since we have our own imputed data in 33 TCGA cancer, Oscar just show me the outline of how to merge all the signatures and do linear model analysis, I am starting learing all the assocsion analysis , I'll discuss with him to ensure that you are proceeding correctly.


- besdies taht, I am also using Michigan imputatoin server for our 


- Also start doing genotyping in Burket's lymphpma dataset and will merge the data once finished. 
- 
- ICGC WGS
WES

- GSTM1 bam files is stranded lib prepared, probably could take a look
---

### 0607

- Doing a lot of genotype
- Finished Burkets lymopha currently combined the gene TPM into that file

- downloaded the TCGA bladder cancer WGS data, current finish around 20 sample,
- meanwhile I also slice chr5 for genotyping the SNPs of TERT and compare the result with Michigan and our pipeline, I generate the argeement plots 

- will compare the result with 2 years ago 
- 
eqtl
...
- For Burkets Lymopma data, finished combined the TMP to genotype, I put the cvs file in the same folder.

- finished downloaded all the normal TCGA bladder cancer WGS bam files, it's now in T-drvie TCGA folder

-  for testing imputation result, Mila suggest using the T4 proxy SNPs which is on the exon, and TCGA have most wxs data, for comparing the result. I dowlaoded it and GT this SNPs.  

- Next goal is to Generate consense GT of proxy_SNPs and target SNPs, and do lm() to see if the result are comparable.
 
#Currently also downloading wxs of other cancer type so that we'll have this more accurate GT

---


- For HiCHip, after discussed with Mai, she suggested that I can run bioanalizer to determine the library size and provide the information in the sample sheet. I'll do bioanalyzer and then mail to Kristine about the submit sample

- Order record/ CGR request/ in L-drive
- update project once done 
- eqtl gene expression normalization when is right
- do GT vs lm() stragify subtype, age,etc...

- ATAC-seq add to uscs

---

### 0621

- just focus on Burks dataset, just finishing download and upload bam slice to biowulf.
- I also finished gt of new  set of SNP around chr6 and 14, should I merge all these gt result to the previous chr5 and chr21 result with TPM?
- Tomorrow test bioanalyzer, and once done can prepare sample to CGR
